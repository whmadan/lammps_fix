  ...
  fixrigid(NULL), fixshake(NULL), random(NULL)
{
  ...
  // required args add two args before to read current and lower critical density (cdensity and lcd)

 cdensity = force->inumeric(FLERR,arg[3]);
  lcd = force->inumeric(FLERR,arg[4]);
  ninsert = force->inumeric(FLERR,arg[5]);
  ntype = force->inumeric(FLERR,arg[6]);
...
/* ----------------------------------------------------------------------
   perform particle insertion or deletion
------------------------------------------------------------------------- */

void FixDeposit::pre_exchange()
{
  int i,j,m,n,nlocalprev,imol,natom,flag,flagall,iwhichglobal,iwhichlocal;
  int ndel,ndeltopo[4];
